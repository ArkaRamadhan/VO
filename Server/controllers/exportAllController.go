	...
	for _, sheetName := range sheetNames {
		...
		for i, dataRow := range dataRows {
			...
			case "MEETING SCHEDULE":
				...
				f.SetCellValue(sheetName, fmt.Sprintf("B%d", rowNum), tanggalString)
				...
				case "Done":
					f.SetCellStyle(sheetName, fmt.Sprintf("G%d", rowNum), fmt.Sprintf("G%d", rowNum), styleDone)
				case "Cancel":
					f.SetCellStyle(sheetName, fmt.Sprintf("G%d", rowNum), fmt.Sprintf("G%d", rowNum), styleCancel)
				case "Reschedule":
					f.SetCellStyle(sheetName, fmt.Sprintf("G%d", rowNum), fmt.Sprintf("G%d", rowNum), styleReschedule)
				case "On Progress":
					f.SetCellStyle(sheetName, fmt.Sprintf("G%d", rowNum), fmt.Sprintf("G%d", rowNum), styleOnProgress)
				}
				...
				err = f.SetCellStyle("MEETING SCHEDULE", "A2", "H"+strconv.Itoa(len(meetingLists)+1), styleAll)
...
func UpdateAllSheets(c *gin.Context) {
	...
	for _, sheetName := range sheetNames {
		...
		case "MEETING SCHEDULE":
			...
			f.SetCellValue(sheetName, "C1", "Perihal")
			f.SetCellValue(sheetName, "D1", "Waktu")
			f.SetCellValue(sheetName, "E1", "asod")
			f.SetCellValue(sheetName, "F1", "Tempat")
			f.SetCellValue(sheetName, "G1", "Status")
			f.SetCellValue(sheetName, "H1", "PIC")
