	...
	for r := 4 + rowOffset; day <= daysInMonth; r += 2 {
		...
		for d := firstDay; d < 7 && day <= daysInMonth; d++ {
			...
			for _, event := range events {
				// Sesuaikan format parsing dengan format yang ada di database
				...
				if (currentDate.Equal(startDate) || currentDate.After(startDate)) && currentDate.Before(endDate.AddDate(0, 0, 1)) {
					// Konversi event.ResourceId dari int ke uint
					resourceName := resourceMap[uint(event.ResourceId)]
					// Simpan ResourceName di kolom yang sama dengan tanggal
					week[d] = fmt.Sprintf("%d %s", day, resourceName)
					// Simpan judul event di baris kosong di bawah tanggal
